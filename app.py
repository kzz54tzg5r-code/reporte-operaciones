...
def get_operational_data():
    try:
        # Enlace de descarga directa de tu archivo en OneDrive
        URL_ONEDRIVE = "https://onedrive.live.com/download?resid=11B83163-6E2D-4B29-A4C2-9D0A3BB17B97"
        
        # Cabeceras en texto plano para simular peticiones de un navegador web estándar
        config_conexion = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3"
        }
        
        # Lectura del archivo alojado en la nube
        df = pd.read_excel(URL_ONEDRIVE, storage_options=config_conexion)
        ...
        st.error(f"Error al conectar con la base de datos de OneDrive: {e}")
        return pd.DataFrame()
...
# --- VALIDACIÓN DE CARGA ANTES DE RENDERIZAR INTERFAZ ---
if not df_master.empty:
    ...
    if not df_filtered.empty:
        ...
        # Extraer de forma automatizada los últimos 4 bloques semanales registrados
        ...
        with tab_auditoria:
            # Visualizaciones que abarcan todo el ancho disponible de pantalla
    ...
    st.info("Estructura base montada. El sistema leerá automáticamente las columnas ('Mes', 'Semana', 'Fecha', 'Tienda', 'Sis_Aduana', 'Fis_Aduana', 'Muertos', 'Cajas', 'Meta_Rec', 'Real_Rec', 'Habilitadas', 'Ubicadas') de tu hoja activa de OneDrive.")
